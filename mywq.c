...
static char *keyboard[84] =  { " ", "Esc", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "+", "Backspace", 
...
static void mywq_fun_1(struct work_struct *work)
{
    ...
    if (mywork->code & 0x80  && ((mywork->code & 0x7f) < 84))
        printk(KERN_INFO "^ mywq_fun_1: time = %llu, code = %s\n", ktime_get(), keyboard[mywork->code & 0x7f]);
}
...
static void mywq_fun_2(struct work_struct *work)
{
    ...
    if (mywork->code & 0x80  && ((mywork->code & 0x7f) < 84))
    {
        printk(KERN_INFO "^ mywq_fun_2: time = %llu, delay = 100\n", ktime_get());
        msleep(100);
        printk(KERN_INFO "^ mywq_fun_2: time = %llu, code = %s\n", ktime_get(), keyboard[mywork->code & 0x7f]);
    }
}
...
static int __init mywq_init(void)
{
    ...
    if (!mywq)
    {
        ...
        printk(KERN_ERR "^ create_workqueue\n");
        return -ENOMEM;
    }
    ...
    {
        free_irq(IRQ_NO, (void *)(my_irq_handler));
        printk(KERN_ERR "^ kmalloc work_1\n");
        return -ENOMEM;
    }
    ...
    if (!work_2)
    {
        free_irq(IRQ_NO, (void *)(my_irq_handler));
        kfree(work_1);
        printk(KERN_ERR "^ kmalloc work_2\n");
        return -ENOMEM;
    }
    ...
    if (!proc_file) {
        printk(KERN_ERR "^ can't proc_create\n");
        return -ENOMEM;
    }
    ...
    if (!proc_dir || !proc_link) {
        printk(KERN_ERR "^ can't proc_mkdir or proc_symlink\n");
        return -ENOMEM;
    }
    
    printk(KERN_INFO "^ init mywq\n");
